...
# installed.
#
# Note: the current working directory has been set by prep_node to the directory
...
# copy_hdp_repo: copy the repo file to the correct location.
#
function copy_hdp_repo(){
 
  local REPO='hdp.repo'; local REPO_DIR='/etc/yum.repos.d'
  local out; local err
  ...
  err=$?
  display "HDP repo cp: $out" $LOG_DEBUG
  if (( err != 0 )) ; then
    display "ERROR: HDP repo copy $err" $LOG_FORCE
    exit 5 
  fi
}
...
function install_ambari_server(){
  ...
  # is redirected to disk you get a *very* long record.
...
function install_common(){
  ...
  # copy HDP repo
  echo
  display "-- Copying HDP repo file" $LOG_SUMMARY
  copy_hdp_repo
